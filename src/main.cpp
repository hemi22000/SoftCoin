...
uint256 hashGenesisBlock("0x2f525e81a2860c7da5e8b1183c00fe0b194e1814b5d5e1cfe5642f59d36d6554");
...
{
    // SoftCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // SoftCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "softcoin genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
	    ...
	    assert(block.hashMerkleRoot == uint256("0x7d6d38591d494147dee5accb7f40ba3b3ebdf964fdec9c26579ccab08870ffd5"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SoftCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SoftCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SoftCoinMiner(CWallet *pwallet)
{
    printf("SoftCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("SoftCoin-miner");
    ...
    try { loop {
        ...
        printf("Running SoftCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SoftCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SoftCoinMiner, pwallet));
}
