...
// Copyright (c) 2011-2012 SoftCoin Developers
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #SoftCoinTEST0\r");
            Send(hSocket, "WHO #SoftCoinTEST0\r");
        } else {
            // randomly join #SoftCoin00-#SoftCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #SoftCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #SoftCoin%02d\r", channel_number).c_str());
        }
